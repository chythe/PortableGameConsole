/**************************************************************************//**
* @file     Font.c
* @brief    Czionka i zarzadzanie tekstami - souce file
...
/**************************************************************************//**
 * @brief Wyswietla znak.
 * @param character - wyswietlany znak
 * @param colorFront -	kolor czcionki
 * @param colorBackground - kolor tla
 * @param x - wspolrzedna x lewego gornego konca znaku
 * @param y - wspolrzedna y lewego gornego konca znaku
 *****************************************************************************/
void FontDisplayChar(char character, uint16_t colorFront, uint16_t colorBackground, uint8_t x, uint16_t y)
{
	uint8_t i, j, value, mask;
	...
	LCDWriteReg(INDEX_HORIZONTAL_RAM_ADDRESS_END_POSITION, x + CHAR_MAX_X - 1);
	LCDWriteReg(INDEX_VERTICAL_RAM_ADDRESS_END_POSITION, y + CHAR_MAX_Y - 1);
	...
	for (j = 0; j < CHAR_MAX_Y; j++)
	{
		mask = CHAR_MASK;
		value = (uint8_t)pgm_read_byte(&ascii[character * CHAR_MAX_Y + j]);
		for (i = 0; i < CHAR_MAX_X; i++)
		{
			if (value & mask)
				LCDWriteData(colorFront);
			else
				LCDWriteData(colorBackground);
			mask >>= 1;
		}
	}
...
/**************************************************************************//**
 * @brief Wyswietla napis.
 * @param string - wyswietlany napis
 * @param colorFront -	kolor czcionki
 * @param colorBackground - kolor tla
 * @param x - wspolrzedna x lewego gornego konca napisu
 * @param y - wspolrzedna y lewego gornego konca napisu
 *****************************************************************************/
void FontDisplayString(char* string, uint16_t colorFront, uint16_t colorBackground, uint8_t x, uint16_t y)
{
	while (*string)
	{
		if (y > STRING_MAX_Y - CHAR_MAX_X)
			break;
		if (x > STRING_MAX_X - CHAR_MAX_X)
		{
			x = 0;
			y += CHAR_MAX_Y;
		}

		FontDisplayChar(*string, colorFront, colorBackground, x, y);
